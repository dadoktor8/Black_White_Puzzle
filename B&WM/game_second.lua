...
local Trap_White_SS = graphics.newImageSheet( "Trap_White.png", Trap_White_SS_Opt )
local Trap_White_Idle_Seq = {
    {
        name = "idle",
        start = 1,
        count = 4,
...
local function onLocalCollision(self, event)
    ...
    if(event.phase == "began" and (self.isVisible == true and event.other.isVisible == true)) then
        ...
        if(self.myName == "PlayerWhite" and event.other.myName == "TrapWhite") then
            composer.gotoScene( "gameover",{time = 800, effect = "crossFade"}) 
        end 
    end

end
local function Trap_Placement()
    if(level == 11) then
        trap_white_list[1].isVisible = true
        trap_white_list[1].x = display.contentCenterX + 100
        trap_white_list[1].y = display.contentCenterY - 100
        obj_white.x = display.contentCenterX + 100
        obj_white.y = display.contentCenterY - 150
        for i = 2, 5 do
            trap_white_list[i].isVisible = false 
        end
    elseif(level == 12) then
        trap_white_list[1].isVisible = true
        trap_white_list[1].x = display.contentCenterX + 100
        trap_white_list[1].y = display.contentCenterY - 100
        trap_white_list[2].isVisible = true
        trap_white_list[2].x = display.contentCenterX + 200
        trap_white_list[2].y = display.contentCenterY - 100
        obj_white.x = display.contentCenterX + 150
        obj_white.y = display.contentCenterY - 150
        for i = 3, 5 do
            trap_white_list[i].isVisible = false 
        end
    elseif(level == 13) then
        ...
        trap_white_list[2].x = display.contentCenterX + 200
        trap_white_list[2].y = display.contentCenterY - 100
        trap_white_list[3].isVisible = true
        trap_white_list[3].x = display.contentCenterX + 50
        trap_white_list[3].y = display.contentCenterY 
        obj_white.x = display.contentCenterX + 320
        obj_white.y = display.contentCenterY - 150
        for i = 4, 5 do
            trap_white_list[i].isVisible = false 
        end
    elseif(level == 14) then
        ...
        trap_white_list[3].x = display.contentCenterX + 50
        trap_white_list[3].y = display.contentCenterY 
        trap_white_list[4].isVisible = true
        trap_white_list[4].x = display.contentCenterX + 200
        trap_white_list[4].y = display.contentCenterY + 100
        obj_white.x = display.contentCenterX + 50
        obj_white.y = display.contentCenterY - 150
    elseif(level == 15) then
        ...
        trap_white_list[4].x = display.contentCenterX + 200
        trap_white_list[4].y = display.contentCenterY + 100
        trap_white_list[5].isVisible = true
        trap_white_list[5].x = display.contentCenterX + 250
        trap_white_list[5].y = display.contentCenterY + 100
        obj_white.x = display.contentCenterX + 200
        obj_white.y = display.contentCenterY - 150
...
function scene:create( event )
   ...
   -- obj_white:setFillColor(1,1,1)
    ...
    trap_white_list = {}
    for i = 1, 5 do 
        trap_white =display.newSprite( mainGroup, Trap_White_SS, Trap_White_Idle_Seq )
        trap_white.xScale = 2
        trap_white.yScale = 2
        trap_white.anchorX = 0.5
        trap_white.anchorY = 0.5
        trap_white.width= 20
        trap_white.height= 20
        trap_white.isVisible = false
        trap_white.myName = "TrapWhite"
        physics.addBody( trap_white, "kinematic", {density = 0, friction = 0} )
        table.insert( trap_white_list, trap_white)
    end
